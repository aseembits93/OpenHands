...
class LLM(RetryMixin, DebugMixin):
    ...
    def __init__(
        ...
        self.tokenizer = create_pretrained_tokenizer(self.config.custom_tokenizer) if self.config.custom_tokenizer is not None else None
        ...
        config_model_lower = self.config.model.lower()
        if 'mistral' in config_model_lower and self.config.safety_settings:
            kwargs['safety_settings'] = self.config.safety_settings
        elif 'gemini' in config_model_lower and self.config.safety_settings:
            kwargs['safety_settings'] = self.config.safety_settings
        ...
        if 'claude-opus-4-1' in config_model_lower:
            kwargs['thinking'] = {'type': 'disabled'}

        if ('claude-opus-4-1' in config_model_lower) and (
        ...
        # Save unwrapped completion (avoid potential attribute lookup overhead in wrapper)
        _completion_unwrapped = self._completion
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            # Avoid list construction if possible, and avoid double work for non-list, non-Message cases
            messages_kwarg: (
            ...
            if len(args) > 1:
                messages_kwarg = args[1]
                kwargs['messages'] = messages_kwarg
                args = args[2:]
            elif 'messages' in kwargs:
                messages_kwarg = kwargs['messages']

            # Efficiently format to Message list only if needed, and avoid unnecessary copies
            if isinstance(messages_kwarg, list):
                if messages_kwarg and isinstance(messages_kwarg[0], Message):
                    messages: list[dict] = self.format_messages_for_llm(cast(list[Message], messages_kwarg))
                else:
                    messages = cast(list[dict[str, Any]], messages_kwarg)
            else:
                if isinstance(messages_kwarg, Message):
                    messages = self.format_messages_for_llm([messages_kwarg])
                else:
                    messages = [messages_kwarg]

            kwargs['messages'] = messages

            # Only create deep copies if mock_function_calling required
            if mock_function_calling and 'tools' in kwargs:
                add_in_context_learning_example = not (
                    'openhands-lm' in self.config.model or 'devstral' in self.config.model
                )

                # Only copy messages for logging if needed
                ...
                features = get_features(self.config.model)
                # Fewer lookups
                if features.supports_stop_words and not self.config.disable_stop_word:
                ...
                # Avoid unnecessary copy if not needed
            ...
            with warnings.catch_warnings():
                ...
                resp: ModelResponse = _completion_unwrapped(*args, **kwargs)

            latency = time.time() - start_time
            response_id = resp.get('id', 'unknown')
            self.metrics.add_response_latency(latency, response_id)

            # Only deep copy if needed
            non_fncall_response = copy.deepcopy(resp) if mock_function_calling and mock_fncall_tools is not None else None
    ...
    def is_function_calling_active(self) -> bool:
        ...
        """
        # Already constant-time: just be explicit about direct member access
